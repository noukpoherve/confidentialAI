...
- Name: test-runner.spec.ts >> dashboard test runner >> starts a single scenario run from row action
- Location: tests/e2e/test-runner.spec.ts:18:7
...
Locator:  locator('tbody tr').first().getByRole('button', { name: 'Run' })
Expected: enabled
Received: disabled
Timeout:  240000ms

Call log:
  - Expect "toBeEnabled" with timeout 240000ms
  - waiting for locator('tbody tr').first().getByRole('button', { name: 'Run' })
    232 × locator resolved to <button disabled type="button" class="rounded-lg border border-line bg-canvas px-3 py-1.5 text-xs font-semibold text-ink disabled:cursor-not-allowed disabled:opacity-50">Run</button>
